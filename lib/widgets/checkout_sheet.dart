...
class _CheckoutSheetState extends State<CheckoutSheet> {
  ...
  String get _localPaymentNumber {
    final digits = (paymentNumber ?? widget.store.paymentNumber).replaceAll(
      RegExp(r'[^0-9]'),
      '',
    );
        ...
        children: [
          ...
          Container(
            ...
            decoration: BoxDecoration(
              ...
              border: Border.all(color: const Color(0x33164580)),
            ),
            child: Row(
              children: [
                const CircleAvatar(
          ...
          Row(
            children: [
              ...
              const Spacer(),
              Text(
                formatRwf(widget.store.totalRwf),
  ...
  Widget _paymentStep() => SingleChildScrollView(
    ...
    child: Column(
      ...
      children: [
        const CircleAvatar(
          radius: 34,
          backgroundColor: Color(0xFFE6F2DF),
          child: Icon(Icons.check, color: Color(0xFF2E681D), size: 34),
        ),
        const SizedBox(height: 16),
        const Text(
          'Order received',
        ...
        if (paymentStatus == 'manual') ...[
          ...
          SizedBox(
            ...
            child: OutlinedButton.icon(
              onPressed: busy || paymentNoticeSent ? null : _notifyPaid,
              icon: Icon(
                paymentNoticeSent
          ...
          Text(
            paymentNoticeSent
                ? 'An administrator will verify the transaction and approve your order.'
                : 'Only notify the admin after Mobile Money confirms your transfer.',
            textAlign: TextAlign.center,
            style: const TextStyle(fontSize: 12, color: Colors.black54),
          ),
        ] else ...[
          const Text(
            'Approve the Mobile Money prompt on your phone. The order will update after the payment provider confirms it.',
            textAlign: TextAlign.center,
          ),
        ],
        if (error != null) ...[
  ...
  Future<void> _submit() async {
    if (!widget.auth.authenticated) {
      setState(() => error = 'Please sign in before checkout.');
      return;
    }
    if (location.text.trim().length < 3 ||
        phone.replaceAll(RegExp(r'\D'), '').length < 10) {
      setState(
        () => error = 'Enter a valid delivery location and mobile number.',
      );
      return;
    }
    setState(() {
    ...
    try {
      ...
      setState(() {
        ...
        paymentNumber =
            payment['momoNumber']?.toString() ?? widget.store.paymentNumber;
        complete = true;
      });
      if (paymentStatus == 'manual' && !_isMobileDevice) {
        WidgetsBinding.instance.addPostFrameCallback((_) {
          if (mounted) _showPhoneInstructions();
        });
      }
    } catch (e) {
    ...
    await showDialog<void>(
      ...
      builder: (dialogContext) => AlertDialog(
        ...
        content: Column(
          ...
          children: [
            const Text(
              'This device could not open the MTN MoMo code. On a phone with an MTN SIM, dial:',
            ),
            const SizedBox(height: 14),
            SelectableText(
              _ussdCode,
  ...
  Future<void> _notifyPaid() async {
      ...
      builder: (dialogContext) => AlertDialog(
        ...
        content: SizedBox(
          ...
          child: Column(
            ...
            children: [
              const Text(
                'Confirm only after MTN has completed the transfer. The admin will check the business MoMo account.',
              ),
              const SizedBox(height: 14),
              TextField(
    ...
    if (submitted != true || orderId == null) {
      reference.dispose();
      note.dispose();
      return;
    }
    ...
    try {
      await widget.store.api.notifyManualPayment(
        orderId!,

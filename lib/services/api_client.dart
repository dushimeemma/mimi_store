...
class ApiClient {
  ...
  Future<List<dynamic>> getCategories({bool admin = false}) async =>
      await _request(
        'GET',
  ...
  Future<Map<String, dynamic>> getPublicSettings() async =>
      await _request('GET', '/settings/public', authenticated: false)
  ...
  Future<Map<String, dynamic>> initiatePayment(String orderId) async =>
      await _request('POST', '/orders/$orderId/payment')
          as Map<String, dynamic>;
  Future<void> notifyManualPayment(
    String orderId, {
  ...
  Future<List<dynamic>> myOrders() async =>
      await _request('GET', '/orders/mine') as List<dynamic>;
  Future<List<dynamic>> adminOrders() async =>
      await _request('GET', '/admin/orders') as List<dynamic>;
  Future<Map<String, dynamic>> dashboard() async =>
      await _request('GET', '/admin/dashboard') as Map<String, dynamic>;
  Future<List<Product>> adminProducts() async {
    final data = await _request('GET', '/admin/products') as List<dynamic>;
    return data
        .map((item) => Product.fromJson(item as Map<String, dynamic>))
        .toList();
  }
  ...
  Future<Map<String, dynamic>> uploadProductImage(
    ...
    final request = http.MultipartRequest(
      'POST',
      Uri.parse('$baseUrl/admin/media/product-image'),
    );
    ...
    request.files.add(
      http.MultipartFile.fromBytes(
        ...
        bytes,
        filename: filename,
        contentType: MediaType.parse(mimeType ?? _imageMimeType(filename)),
      ),
    );
    final response = await http.Response.fromStream(await request.send());
    if (response.statusCode == 401 && retry && await _refresh())
      return uploadProductImage(bytes, filename, mimeType, retry: false);
    ...
    if (response.statusCode < 200 || response.statusCode >= 300) {
      ...
      throw ApiException(
        message is List
            ? message.join(', ')
            : (message?.toString() ?? 'Image upload failed'),
        response.statusCode,
      );
    }
  ...
  Future<void> deleteProductImage(String publicId) async {
    await _request(
      'POST',
      '/admin/media/product-image/delete',
      body: {'publicId': publicId},
    );
  }

  Future<void> adjustInventory(String id, int delta, String reason) async {
    await _request(
      'POST',
      '/admin/products/$id/inventory',
      body: {'quantityDelta': delta, 'reason': reason},
    );
  }
  ...
  Future<void> saveCategory(Map<String, dynamic> body, {String? id}) async {
    await _request(
      id == null ? 'POST' : 'PUT',
      id == null ? '/admin/categories' : '/admin/categories/$id',
      body: body,
    );
  }

  Future<List<dynamic>> adminUsers() async =>
      await _request('GET', '/admin/users') as List<dynamic>;
  Future<List<dynamic>> adminDrivers() async =>
      await _request('GET', '/admin/drivers') as List<dynamic>;
  Future<void> updateUserRole(String id, String role) async {
    await _request('PATCH', '/admin/users/$id/role', body: {'role': role});
  }

  Future<void> updateUserStatus(String id, bool active) async {
    await _request(
      'PATCH',
  ...
  Future<List<dynamic>> adminPayments() async =>
      await _request('GET', '/admin/payments') as List<dynamic>;
  Future<void> reviewManualPayment(
    ...
    await _request(
      'POST',
      '/admin/payments/$id/${approved ? 'approve' : 'reject'}',
      body: {if (note?.isNotEmpty == true) 'note': note},
    );
  }

  Future<void> reconcilePayments() async {
    await _request('POST', '/admin/payments/reconcile');
  }

  Future<List<dynamic>> deliveries() async =>
      await _request('GET', '/deliveries') as List<dynamic>;
  ...
  Future<void> updateDelivery(String id, String status, {String? notes}) async {
    await _request(
      'PATCH',
  ...
  Future<void> updateStoreSettings(
    ...
    await _request(
      ...
      body: {'storeName': name, 'supportPhone': phone, 'supportEmail': email},
    );
  }

  Future<List<dynamic>> auditLogs() async =>
      await _request('GET', '/admin/audit-logs') as List<dynamic>;
  Future<void> updateOrderStatus(String id, String status) async {
    await _request('PATCH', '/orders/$id/status', body: {'status': status});
  }
